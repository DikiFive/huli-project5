...
void main()
{
	...
	while (1)
	{
		...
		if (kn.KeyNum == 1 || kn.Num == 1 || kn.Num == 69) //按键1按下
		{
		...
		// UART_SendByte(clock.Sec); //串口发送一个字节
	}
}
...
void clock_set()
{
	...
	if (kn.MatrixKey == 2 || kn.Num == 2 || kn.Num == 96) //按键2按下
	{
	...
	//所在位置时间增加
	if (kn.MatrixKey == 3 || kn.Num == 3) //按键3按下
	{
	...
	//所在位置时间减少
	if (kn.MatrixKey == 4 || kn.Num == 4) //按键4按下
	{
		switch (mod.AlarmSetSelect)
		{
		case 0:
...
void TimeSet()
{
	...
	if (kn.KeyNum == 2 || kn.Num == 2 || kn.Num == 96) //按键2按下
	{
...
void ReadIr() interrupt 0
{
	...
	if (IRIN == 0) //确认是否真的接收到正确的信号
	{
		...
		if (IRs.IrValue[2] != ~IRs.IrValue[3])
		{
			return;
		}
	}
